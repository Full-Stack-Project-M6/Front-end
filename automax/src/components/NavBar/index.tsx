...
export const NavBar = ({variant}:IPropsNavBar) => {
    ...
    return (
        ...
        <StyledNav responsive={menuMobileIsOpen}>
            ...
            {menuMobileIsOpen ? 
              <button className="mobile" onClick={() => setMenuMobileIsOpen((prev) => (!prev))}>
                <IoMdClose/>
              </button>
            :
              <button className="mobile" onClick={() => setMenuMobileIsOpen((prev) => (!prev))}>
            ...
            <div>
                ...
                ?
                (
                <button className="menu" onClick={() => setMenuIsOpen((prev) => (!prev))}>
                  <Announcer/>
                  {menuIsOpen && 
                    <div className="menu__options">
                      <StyledLink to={"/"}><Body1>Editar Perfil</Body1></StyledLink>
                      <StyledLink to={"/"}><Body1>Editar Endereço</Body1></StyledLink>
                      <StyledLink to={"/"}><Body1>Meus Anúncios</Body1></StyledLink>
                      <button><Body1>Sair</Body1></button>
                    </div>
                  }
